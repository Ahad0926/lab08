...
def test_create_quiz(mock_create_quiz, client):
    ...
    response = client.post(
        '/api/quizzes',
        json={"title": "Sample Quiz", "questions": [{"text": "Q1?", "answer": "A1"}]}
    )

    assert response.status_code == 201
    assert response.json == {"message": "Quiz created", "quiz_id": 1}
    mock_create_quiz.assert_called_once_with("Sample Quiz", [{"text": "Q1?", "answer": "A1"}])


@patch.object(QuizService, 'get_quiz')
def test_get_quiz(mock_get_quiz, client):
    mock_get_quiz.return_value = type('MockQuiz', (object,), {"id": 1, "title": "Sample Quiz", "questions": []})
